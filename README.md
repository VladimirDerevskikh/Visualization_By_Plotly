...
Unfortunately for now it's not possible to display such big files in GitHub as my data (csv-file) and graph (html-file) so to have a look at my data or graph you'll need to download it (but there is also a jpg-file in this repository which demonstrates how does the graph looks like).
